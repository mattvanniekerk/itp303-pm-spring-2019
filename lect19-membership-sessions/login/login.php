<?php
session_start();
// session_start whenever we use a session. 
...
require '../config/config.php';
	...
	if ( isset($_POST['username']) && isset($_POST['password']) ) {
		...
		if ( empty($_POST['username']) || empty($_POST['password']) ) {
			...
			// Hash the password user just submitted. Then compare that hashed password to the hashed password stored in the DB.
			$passwordInput = hash('sha256', $_POST['password']);

			// SQL: look for username with the pw.
